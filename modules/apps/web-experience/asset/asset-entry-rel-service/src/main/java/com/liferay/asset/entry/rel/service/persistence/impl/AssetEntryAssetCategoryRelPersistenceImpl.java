		...
		if (result == null) {
			...
			try {
				...
				if (list.isEmpty()) {
					...
					if ((assetEntryAssetCategoryRel.getAssetEntryId() != assetEntryId) ||
							(assetEntryAssetCategoryRel.getAssetCategoryId() != assetCategoryId)) {
						finderCache.putResult(FINDER_PATH_FETCH_BY_A_A,
							finderArgs, assetEntryAssetCategoryRel);
					}
				}
			}
			catch (Exception e) {

...
public class FragmentEntryServiceImpl extends FragmentEntryServiceBaseImpl {
	...
	public BaseModelSearchResult<FragmentEntry> searchFragmentEntries(
			long groupId, long fragmentCollectionId, String title, int start,
			int end, Sort sort)
		throws PortalException {

		User user = getUser();

		return fragmentEntryLocalService.searchFragmentEntries(
			user.getCompanyId(), groupId, fragmentCollectionId, title, start,
			end, sort);
	}

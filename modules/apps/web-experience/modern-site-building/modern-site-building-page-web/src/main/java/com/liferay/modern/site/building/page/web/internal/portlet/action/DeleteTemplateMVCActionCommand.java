...
@Component(
	...
	property = {
		...
		"mvc.command.name=/page_template/delete_template"
	},
...
public class DeleteTemplateMVCActionCommand extends BaseMVCActionCommand {
	...
	protected void doProcessAction(
		...
		long[] msbPageTemplateIds = null;

		long msbPageTemplateId = ParamUtil.getLong(
			actionRequest, "msbPageTemplateId");

		if (msbPageTemplateId > 0) {
			msbPageTemplateIds = new long[] {msbPageTemplateId};
		}
		else {
			msbPageTemplateIds = ParamUtil.getLongValues(
				actionRequest, "rowIds");
		}

		for (long deleteMSBPageTemplateId : msbPageTemplateIds) {
			_msbPageTemplateService.deleteMSBPageTemplate(deleteMSBPageTemplateId);
		}

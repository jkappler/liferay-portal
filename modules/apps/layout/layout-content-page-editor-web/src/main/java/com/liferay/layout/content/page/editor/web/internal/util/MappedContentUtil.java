...
public class MappedContentUtil {
	...
	private static JSONObject _getJournalArticleStatusJSONObject
		(long classPK)
	...
	private static Map<String, Object> _getMappedContentJSONObject(
		...
		JSONUtil.put("className", assetEntry.getClassName())

			.put("classNameId", assetEntry.getClassNameId())

			.put("classPK", assetEntry.getClassPK())

			.put(
				"actions",
				_getActionsJSONObject(
					assetEntry, themeDisplay, httpServletRequest))
			...
			.put("title", assetEntry.getTitle(themeDisplay.getLocale()));

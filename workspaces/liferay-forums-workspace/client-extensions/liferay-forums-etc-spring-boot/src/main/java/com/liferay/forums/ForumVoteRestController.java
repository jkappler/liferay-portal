...
public class ForumVoteRestController extends BaseRestController {
	...
	private void _processRecalculateVoteScore(String json, String authToken) {
		...
		long messageId = valuesJSONObject.optLong(
			"r_messageVotes_c_c2m0MessageId", 0L);

		if (messageId == 0L) {
			if (_log.isWarnEnabled()) {
				_log.warn(
					"onRecalculateVoteScore: missing " +
						"r_messageVotes_c_c2m0MessageId in payload");
			}

			return;
		}

		String messageERC = valuesJSONObject.optString(
			"r_messageVotes_c_c2m0MessageERC", "");

		if (messageERC.isEmpty()) {
			if (_log.isWarnEnabled()) {
				_log.warn(
					"onRecalculateVoteScore: missing " +
						"r_messageVotes_c_c2m0MessageERC in payload");
			}

			return;
		}

		long siteId = objectEntryJSONObject.optLong("groupId", 0L);

		_forumVoteService.recalculateVoteScore(
			messageId, messageERC, siteId, authToken);
	}
